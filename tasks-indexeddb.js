...
storageEngine = function (){
	...
	return {
		...
		initObjectStore: function (type, successCallback, errorCallback){
			...
			if (exists){
				...
				request.onupgradeneeded = function (event){
					...
					//objectStore.createIndex("category", "category", {unique: false}); //That's the way to create an index
				};
			}
		...
		save: function (type, obj, successCallback, errorCallback){
			...
			if (!database){
				errorCallback('storage_not_initialized','The storage engine has not been initialized');
			}
			if (!obj.id){
				delete obj.id;
			}
			...
			tx.onerror = function (event){
				errorCallback('transaction_error','It is not possible to complete the transaction');
			};
		...
		findAll: function (type, successCallback, errorCallback){
			...
			request.onerror = function (event){
				errorCallback('failed_to_open_cursor', 'openCursor() failed on findAll');
			};
